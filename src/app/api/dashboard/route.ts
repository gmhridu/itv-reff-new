...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const userPosition = await PositionService.getUserCurrentPosition(
      user.id as string,
    );
    ...
    const tasksCompletedToday = await PositionService.getDailyTasksCompleted(
      user.id as string,
    );
    const canCompleteTask = await PositionService.canCompleteTask(
      user.id as string,
    );
    ...
    const taskEarningsToday = todayTasks.reduce(
      (sum, task) => sum + task.rewardEarned,
      0,
    );

    // Get management bonus stats
    const managementBonusStats =
      await TaskManagementBonusService.getManagementBonusStats(
        user.id as string,
      );

    // Get referral hierarchy stats
    const referralHierarchyStats =
      await EnhancedReferralService.getReferralHierarchyStats(
        user.id as string,
      );
    ...
    const totalEarningsToday =
      incomeStreams.taskIncome +
      incomeStreams.referralRewards +
      incomeStreams.managementBonuses;

    // Calculate simple referral stats for dashboard cards
    const totalReferrals =
      referralHierarchyStats.aLevelCount +
      referralHierarchyStats.bLevelCount +
      referralHierarchyStats.cLevelCount;
    const totalReferralEarnings = referralHierarchyStats.totalEarnings.total;

    // Get all available position levels for task hall
    const allPositions = await PositionService.getAllPositions();
    const availableLevels = allPositions.map((position) => ({
      name: position.name,
      level: position.level,
      isUnlocked: position.level <= (userPosition.position?.level || 0),
    }));
