...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Get recent video tasks for all team members
    const userIds = teamMembers.map((m) => m.user.id);
    const recentVideoTasks = await db.userVideoTask.findMany({
      where: {
        userId: { in: userIds },
        createdAt: {
      ...
      return {
        ...
        currentPosition: user.currentPosition
          ? {
              ...
              tasksPerDay: user.currentPosition.tasksPerDay,
            }
          : null,
        performance: {
    ...
    const teamStats = {
      ...
      veryActiveMembers: formattedTeamMembers.filter(
        (m) => m.activityStatus === "very_active",
      ).length,
      inactiveMembers: formattedTeamMembers.filter(
        (m) => m.activityStatus === "inactive",
      ).length,
      totalTeamEarnings: formattedTeamMembers.reduce(
        (sum, m) => sum + m.performance.totalEarnings,
        0,
      ),
      averageEarningsPerMember:
        formattedTeamMembers.length > 0
          ? formattedTeamMembers.reduce(
              (sum, m) => sum + m.performance.totalEarnings,
              0,
    ...
    const positionDistribution = formattedTeamMembers.reduce(
      (acc, member) => {
    ...
    const topPerformers = [...formattedTeamMembers]
      .sort(
        (a, b) => b.performance.weeklyEarnings - a.performance.weeklyEarnings,
      )
      .slice(0, 5);
