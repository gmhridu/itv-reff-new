...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const conversionFunnel = {
      ...
      registrations: referralActivities.filter(a => a.status === 'REGISTERED' || a.status === 'QUALIFIED' || a.status === 'REWARDED').length,
      qualifications: referralActivities.filter(a => a.status === 'QUALIFIED' || a.status === 'REWARDED').length,
      rewards: referralActivities.filter(a => a.status === 'REWARDED').length
    };

    // Calculate conversion rates
    const conversionRates = {
      clickToRegistration: conversionFunnel.clicks > 0 ? (conversionFunnel.registrations / conversionFunnel.clicks) * 100 : 0,
      registrationToQualification: conversionFunnel.registrations > 0 ? (conversionFunnel.qualifications / conversionFunnel.registrations) * 100 : 0,
      qualificationToReward: conversionFunnel.qualifications > 0 ? (conversionFunnel.rewards / conversionFunnel.qualifications) * 100 : 0,
      overallConversion: conversionFunnel.clicks > 0 ? (conversionFunnel.rewards / conversionFunnel.clicks) * 100 : 0
    };
    ...
    for (let i = 0; i < periodDays; i++) {
      ...
      dailyAnalytics.push({
        ...
        registrations: dayActivities.filter(a => a.status === 'REGISTERED' || a.status === 'QUALIFIED' || a.status === 'REWARDED').length,
        earnings: (dayEarnings._sum.amount || 0) + (dayManagementBonuses._sum.bonusAmount || 0)
      });
    }

    // Get traffic source analytics
    const trafficSources = referralActivities.reduce((acc, activity) => {
    ...
    const performanceMetrics = {
      averageDailyClicks: dailyAnalytics.reduce((sum, day) => sum + day.clicks, 0) / periodDays,
      averageDailyRegistrations: dailyAnalytics.reduce((sum, day) => sum + day.registrations, 0) / periodDays,
      averageDailyEarnings: dailyAnalytics.reduce((sum, day) => sum + day.earnings, 0) / periodDays,
      bestPerformingDay: dailyAnalytics.reduce((best, current) => 
        current.earnings > best.earnings ? current : best, 
        { date: '', earnings: 0 }
      ),
      totalPeriodEarnings: dailyAnalytics.reduce((sum, day) => sum + day.earnings, 0)
    };
