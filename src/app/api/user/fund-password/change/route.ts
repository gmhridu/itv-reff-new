...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { currentFundPassword, fundPassword } = body;
    ...
    // Get user with fund password from database using raw query
    ...
    if (!userData[0].fundPassword) {
      response = NextResponse.json(
        { error: "No fund password set. Use set endpoint instead." },
        { status: 400 },
      );
      return addAPISecurityHeaders(response);
    }

    // If current fund password is provided, verify it
    if (currentFundPassword) {
      ...
      if (!isCurrentPasswordValid) {
        response = NextResponse.json(
          { error: "Invalid current fund password" },
          { status: 400 },
        );
        return addAPISecurityHeaders(response);
      }
      ...
      if (isSamePassword) {
        response = NextResponse.json(
          {
            error: "New fund password must be different from current password",
          },
          { status: 400 },
        );
        return addAPISecurityHeaders(response);
    ...
    // Log the fund password change activity
    await prisma.activityLog.create({
      data: {
        userId: user.id,
        activity: "fund_password_change",
        description: "User changed their fund password",
        ipAddress:
    ...
    response = NextResponse.json({
      success: true,
      message: "Fund password changed successfully",
    });
