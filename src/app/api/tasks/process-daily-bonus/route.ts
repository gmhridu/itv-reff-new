...
// PATCH endpoint to control scheduler (start/stop)
  ...
  try {
    ...
    switch (action) {
      case "start":
        ...
        result = { message: "Scheduler started" };
        break;
      case "stop":
        ...
        break;
      case "status":
        result = dailyTaskScheduler.getStatus();
        break;
    }
...
// PUT endpoint for manual processing trigger
export async function PUT(request: NextRequest) {
  try {
    ...
    const result = await dailyTaskScheduler.triggerManualProcessing(date ? new Date(date) : undefined);

    // Log manual trigger to database
    await db.systemLog.create({
      data: {
        level: "INFO",
        component: "DAILY_TASK_SCHEDULER",
        message: `Manual daily task processing triggered`,
        metadata: JSON.stringify({
